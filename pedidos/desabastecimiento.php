...
$depoMarketPlace = $_SESSION['deposi'];
...
if(!isset($_SESSION['username'])){
	...
	if($_SESSION['nuevoPedido']==0 && $_SESSION['cargaPedido']==1){
		...
		$sql="
			...
			(
				...
				(
					...
					UNION ALL
					SELECT B.COD_ARTICU, SUM(B.CANT_PEDID) CANT_PEDID FROM GVA21 A INNER JOIN GVA03 B ON A.NRO_PEDIDO = B.NRO_PEDIDO AND A.TALON_PED = B.TALON_PED
					WHERE A.TALON_PED = 97 AND A.COD_SUCURS = '$depoMarketPlace' AND ESTADO = 2 AND B.COD_ARTICU NOT LIKE '[*]%' AND A.FECHA_PEDI >= GETDATE()-15 GROUP BY B.COD_ARTICU
					)A GROUP BY A.COD_ARTICU
				)C
				ON B.COD_ARTICU = C.COD_ARTICU
				INNER JOIN SOF_RUBROS_TANGO D
				ON A.COD_ARTICU = D.COD_ARTICU
				WHERE A.COD_DEPOSI = '$depoMarketPlace'
				AND A.CANT_STOCK > 0
		...
		<form method="POST" action="Controlador/cargarPedidoDesabastecimiento.php" onkeypress = "return pulsar(event)">
