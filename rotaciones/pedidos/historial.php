...
$sql=
	...
	INNER JOIN
	(
	SELECT NRO_PEDIDO, CAST(SUM(CANT_PEDID) AS FLOAT) CANT FROM GVA03 WHERE TALON_PED IN (96) GROUP BY NRO_PEDIDO
	)B
	ON A.NRO_PEDIDO = B.NRO_PEDIDO
	WHERE COD_CLIENT = '$codClient' AND FECHA_PEDI > (GETDATE()-60) AND A.TALON_PED IN (96)
	AND A.LEYENDA_1 LIKE '%ROTACION%'
