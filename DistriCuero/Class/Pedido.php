...
class Pedido
{
    ...
    public function traerDetallePedidosCom($orden){
        ...
        $sql = "SELECT ISNULL(B.FECHA, '') FECHA, NRO_NOTA_PEDIDO, ISNULL(B.COD_CLIENT, A.COD_CLIENT) COD_CLIENT, B.COD_ARTICU, B.DESCRIPCIO, B.CANTIDAD,
                ...
                FROM [LAKERBIS].LOCALES_LAKERS.DBO.SUCURSALES_LAKERS A
				 LEFT JOIN
				 (
				 SELECT FECHA, NRO_ORDEN, NRO_NOTA_PEDIDO, COD_CLIENT, COD_ARTICU, DESCRIPCIO, CANTIDAD FROM RO_PEDIDO_PRECOMPRA
				 WHERE NRO_ORDEN = '$orden'
				 ) B
				 ON A.COD_CLIENT = B.COD_CLIENT
                 LEFT JOIN (SELECT * FROM RO_ORDENES_RECHAZADAS WHERE NRO_ORDEN = '$orden') C ON A.COD_CLIENT = C.COD_CLIENT
				 WHERE A.CANAL = 'FRANQUICIAS' AND A.HABILITADO = 1 AND A.NRO_SUC_MADRE IS NULL
        "
        ;
