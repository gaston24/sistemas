...
$sql = "SELECT C.NOMBRE, C.APELLIDO, 
		COD_VENDED BLOQUE, B.DESC_SUCURSAL, CA_1118_NUM_SUCURSAL NRO_SUCURSAL FROM GVA23 A
		INNER JOIN [LAKERBIS].LOCALES_LAKERS.DBO.SUCURSALES_LAKERS B ON A.CA_1118_NUM_SUCURSAL = B.NRO_SUCURSAL
		INNER JOIN [TANGO-SUELDOS].LAKERS_CORP_SA.DBO.LEGAJO C ON A.COD_VENDED = C.BLOQUE COLLATE Latin1_General_BIN
		WHERE CA_1118_NUM_SUCURSAL IS NOT NULL AND CA_1118_NUM_SUCURSAL = $nroSucurs
		ORDER BY APELLIDO
";
