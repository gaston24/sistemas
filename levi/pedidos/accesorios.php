...
if(!isset($_SESSION['username'])){
	...
	header("Location:../../login.php");
		...
		SELECT * FROM 
			...
			CASE WHEN A.[851_STOCK] IS NULL THEN 0 ELSE A.[851_STOCK] END [851_STOCK], 
			CASE WHEN A.[851_VENDIDO] IS NULL THEN 0 ELSE A.[851_VENDIDO] END [851_VENDIDO], 
			CASE WHEN A.[833_STOCK] IS NULL THEN 0 ELSE A.[833_STOCK] END [833_STOCK], 
			CASE WHEN A.[833_VENDIDO] IS NULL THEN 0 ELSE A.[833_VENDIDO] END [833_VENDIDO], 
			CASE WHEN A.[903_STOCK] IS NULL THEN 0 ELSE A.[903_STOCK] END [903_STOCK], 
			CASE WHEN A.[903_VENDIDO] IS NULL THEN 0 ELSE A.[903_VENDIDO] END [903_VENDIDO], 
			...
			(	
				...
				B.RUBRO, CAST(CANT_STOCK AS INT)CANT_STOCK, A.[851_STOCK], A.[851_VENDIDO], A.[833_STOCK], A.[833_VENDIDO], A.[903_STOCK], A.[903_VENDIDO], 	
				CAST(C.CANT_PEDID AS INT) DIST 
				FROM
				(
					SELECT COD_ARTICU, DESCRIPCIO, CASE WHEN CANT_STOCK < 0 THEN 0 ELSE CANT_STOCK END CANT_STOCK, A.[851_STOCK], A.[851_VENDIDO], A.[833_STOCK], A.[833_VENDIDO], 
					A.[903_STOCK], A.[903_VENDIDO]
					FROM
					(
						...
						A.[851_STOCK], A.[851_VENDIDO], A.[833_STOCK], A.[833_VENDIDO], A.[903_STOCK], A.[903_VENDIDO]
						FROM
						(
							SELECT A.COD_ARTICU, A.DESCRIPCIO, CAST(A.CANT_STOCK AS INT) CANT_STOCK, B.[851_STOCK], B.[851_VENDIDO], B.[833_STOCK], B.[833_VENDIDO], B.[903_STOCK], B.[903_VENDIDO]
							FROM
							...
							AND (A.COD_ARTICU LIKE 'X%' OR A.COD_ARTICU = 'OHGIFT') AND A.CANT_STOCK > 0 )A LEFT JOIN SOF_PEDIDOS_CARGADOS_LEVI B 
		...
		<form method="POST" action="cargarPedidoNuevoLevi.php" onkeypress = "return pulsar(event)">
		<div style="width:100%">
