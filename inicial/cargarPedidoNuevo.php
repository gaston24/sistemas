...
foreach ($_POST['codArt'] as $clave => $valor) {
	...
	if(odbc_num_rows($resultVerifica)==0){
		...
		if($cantArt>0){
			...
			VALUES
			(
			1, $cantArt, $cantArt, $cantArt, $cantArt, $cantArt, '$valor', 0, $nroRenglon, ' '+'$numPed', 0, 0, 0, 97,
			0, 0, 0, 0, 0, 0, 7, 7, 'V', 0,
			0, 0, 0, 0, 0, 0, 0
			)
			";
		...
		if($cantArt>0){
			...
			while($v=odbc_fetch_array($resultExplota)){
				...
				VALUES
				(
				1, $cantArt, $cantArt, $cantArt, $cantArt, $cantArt, '$codArt', 0, $nroRenglon, ' '+'$numPed', 0, 0, 0, 97,
