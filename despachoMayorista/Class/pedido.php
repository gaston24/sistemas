...
class Pedido
{
    ...
    public function traerPedidos($vendedor)
    {
        ...
        $sql = "SELECT A.COD_VENDED, A.COD_CLIENT, A.RAZON_SOCI, A.LOCALIDAD, SUM(A.CANT_PEDIDO) UNID_PEDIDO, SUM(A.CANT_PENDIENTE) UNID_PENDIENTE, COUNT(A.NRO_PEDIDO) CANT_PEDIDOS, SUM(A.IMP_PENDIENTE) IMPORTE FROM RO_PEDIDOS_PENDIENTE_MAYORISTAS A
                ...
                GROUP BY COD_VENDED, A.COD_CLIENT, LOCALIDAD, A.RAZON_SOCI";
    ...
    public function traerPedidosCliente($cliente)
    {
        ...
        $sql = "SELECT FECHA, ESTADO, HORA_INGRESO, COD_CLIENT, RAZON_SOCI, LOCALIDAD, TALON_PED, NRO_PEDIDO, CANT_PEDIDO, CANT_PENDIENTE, IMP_PENDIENTE, COD_VENDED, VENDEDOR, TIPO_COMP, EMBALAJE, DESPACHO, ARREGLO, PRIORIDAD,
                ...
                UNION ALL
                    ...
                    LEFT JOIN SJ_PICKING_ENCABEZADO B ON A.NRO_PEDIDO = B.NRO_PEDIDO AND A.TALON_PED = B.TALON_PED
                    WHERE A.NRO_PEDIDO NOT IN (SELECT NRO_PEDIDO FROM RO_PEDIDOS_MAYORISTA_ASIGNADOS WHERE COD_CLIENT = '$cliente') AND B.IMPRESO = 'NO'
                ) A
