...
define('BASE_URL', $protocol.'://'. $host .'/sistemas');
...
// EXTRAE LA CARPETA DEL MODULO ACTUAL

$pathCompleto = $_SERVER['PHP_SELF'];
$parts = explode("/", $pathCompleto);
$carpetaCorrespondiente = $parts[2];


$folderPath = __DIR__ ;

$files = scandir($folderPath.'/'. $carpetaCorrespondiente);
...
foreach ($files as $file) {
    ...
    if (pathinfo($file, PATHINFO_EXTENSION) === 'css') {
        ...
        if($file == $archivoActual){
            ...
            echo '<link rel="stylesheet" href='.BASE_URL.'/assets/css/'.$carpetaCorrespondiente.'/'.$file.'>';
