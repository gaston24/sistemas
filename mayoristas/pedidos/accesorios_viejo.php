...
if(!isset($_SESSION['username'])){
	...
	if($_SESSION['nuevoPedido']==0 && $_SESSION['cargaPedido']==1){
		...
		$sql="
			...
			SELECT COD_ARTICU, DESCRIPCIO, B.RUBRO, CANT_STOCK FROM
			(
				...
				(
					...
					FROM
						...
						(SELECT A.COD_ARTICU, B.DESCRIPCIO, A.CANT_STOCK FROM
							...
							)A GROUP BY COD_ARTICU )A INNER JOIN STA11 B ON A.COD_ARTICU = B.COD_ARTICU  )A  )A
					LEFT JOIN 
			...
			AND COD_ARTICU NOT IN (SELECT COD_INSUMO FROM STA03 WHERE COD_ARTICU IN ('HC INV 17-01', 'HC INV 17-02','HC VER 17-01-A', 'HC VER 17-01-B', 'HC VER 17-01-C', 'HC VER 17-01-D'))
			ORDER BY 3, 1
